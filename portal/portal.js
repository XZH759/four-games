...
const LOOKS = [
  { id: "explorer", icon: "🦁", key: "portal.profile.look.explorer", bg: "#ffe8c4" },
  { id: "mage", icon: "🦉", key: "portal.profile.look.mage", bg: "#eee3ff" },
  { id: "pilot", icon: "🐶", key: "portal.profile.look.pilot", bg: "#e1f0fa" },
  { id: "artist", icon: "🐰", key: "portal.profile.look.artist", bg: "#fbe4e8" },
];
...
const els = {
  ...
  returningLevel: document.getElementById("returning-level"),
...
function avatarFor(user = state.user) {
  return lookById(user?.avatar_key || user?.profile?.avatar_key || state.avatarKey).icon;
}
...
function paintLooks() {
  const cards = LOOKS.map((look) => {
    ...
    button.innerHTML = `<span style="--look-bg:${look.bg}" aria-hidden="true">${look.icon}</span><strong>${t(look.key)}</strong>`;
    button.addEventListener("click", () => {
      state.avatarKey = look.id;
      paintLooks();
      setText(els.profileAvatar, look.icon);
...
function paintProfile() {
  ...
  setText(els.profileAvatar, lookById(state.avatarKey).icon);
  paintLooks();
  paintBadges();
}
...
function paintReturning() {
  ...
  const castlePercent = Math.round((castleTier / 5) * 100);
  const best = Math.max(0, Number(localStorage.getItem("ailit_parkour_best_v1")) || 0);
  const sprintPercent = Math.min(100, Math.round(best / 25));
  const companion = companionName(user.companion);

  setText(els.returningAvatar, avatarFor(user));
  setText(els.returningName, user.display_name);
  setText(els.returningRole, companion);
  setText(els.returningPoints, points.toLocaleString(getLang() === "en" ? "en-US" : "zh-CN"));
  setText(els.returningLevel, level);
  setText(els.returningLast, relativeLastPlayed(user.previous_login_at || user.logged_in_at));
  setText(els.returningCompanion, companion);
  setText(els.castleChapter, `Chapter ${castleTier}`);
  setText(els.castleProgressLabel, `${castleTier} / 5`);
  els.castleProgress.style.width = `${castlePercent}%`;
  setText(els.castleItems, t("portal.returning.items", { n: inventoryCount(wallet) }));
  setText(els.sprintStage, `Stage ${Math.max(1, Math.ceil(sprintPercent / 25))}`);
  setText(els.sprintProgressLabel, `${sprintPercent}%`);
  els.sprintProgress.style.width = `${sprintPercent}%`;
  setText(els.sprintBest, t("portal.returning.best", { n: best.toLocaleString() }));
}
...
async function submitWelcome(event) {
  ...
  try {
    ...
    setView(result.created ? "profile" : "returning");
  } catch (error) {
...
function updateLoading(progress) {
  ...
  els.syncItems.forEach((item) => {
    item.classList.toggle("is-done", value >= Number(item.dataset.threshold));
  });
}
...
function startLoading({ onboarding = false } = {}) {
  ...
  state.loadingTimer = window.setInterval(() => {
    progress += reduced ? 18 : 2 + Math.random() * 4.2;
...
els.resume.addEventListener("click", () => startLoading({ onboarding: false }));
